...
def build(sites,normal,temperature,bias):
 ...
 if abs((date.fromisoformat(today['date'])-date.fromisoformat(normal['builtFor'])).days)>7:raise ValueError('Seasonal normal expired')
 if not 1 <= (date.fromisoformat(today['date'])-date.fromisoformat(bias['lastDay'])).days <=14:raise ValueError('Calibration expired or overlaps forecast day')
